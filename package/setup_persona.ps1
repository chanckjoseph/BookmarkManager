...
# This script initializes a project with the "Architect" persona and the standard Multi-Agent Workflow for Windows users.

Write-Host "Initializing Multi-Agent Workflow artifacts..."

# 1. Create Directories
$dirs = @(".agent/workflows", ".github")
foreach ($dir in $dirs) {
...
'@
$personaContent | Out-File -FilePath ".agent/persona.md" -Encoding utf8

# 3. Create Workflows
$workflows = @{
    "architect_planning.md" = @'
---
...
foreach ($wf in $workflows.Keys) {
    $workflows[$wf] | Out-File -FilePath ".agent/workflows/$wf" -Encoding utf8
}
...
# 5. Finishing
Write-Host "Setup Complete! The Architect persona is now active."
Write-Host "Workflow documentation can be initialized at docs/process/workflow.html if needed."
